    ...
    if (!forceRefresh) {
      ...
      if (cachedStr != null) {
        try {
          ...
          // Cache for 12 hours
          if (DateTime.now().millisecondsSinceEpoch - timestamp < 12 * 60 * 60 * 1000) {
...
class ConverterNotifier extends Notifier<ConverterState> {
  ...
  void setCategory(FfiConverterCategory category) {
    if (category.id == 'currency') {
      ...
      if (state.currencyRates.isNotEmpty) {
        final units = state.currencyRates.keys.map((k) => FfiUnit(id: k, name: k, symbol: k, multiplier: 1.0, offset: 0.0)).toList();
        final cat = FfiConverterCategory(id: 'currency', name: 'Currency', iconName: 'currency_exchange', units: units);
        state = state.copyWith(
          category: cat,
          fromUnit: units.isNotEmpty ? units.first : null,
          toUnit: units.length > 1 ? units[1] : (units.isNotEmpty ? units.first : null),
          inputValue: '',
          resultValue: '',
        );
      } else {
        state = state.copyWith(category: category, inputValue: '', resultValue: '');
      }
    } else {
      state = state.copyWith(
        category: category,
        fromUnit: category.units.isNotEmpty ? category.units.first : null,
        toUnit: category.units.length > 1 ? category.units[1] : (category.units.isNotEmpty ? category.units.first : null),
        inputValue: '',
        resultValue: '',
        discountPercentage: '',
        gstPercentage: '',
  ...
  void onDelete() {
    if (state.inputValue.isNotEmpty) {
      state = state.copyWith(inputValue: state.inputValue.substring(0, state.inputValue.length - 1));
      _calculateResult();
    }
  }
  ...
  void _calculateResult() {
    ...
    if (catId == 'currency') {
      ...
      if (from != null && to != null && state.currencyRates.containsKey(from) && state.currencyRates.containsKey(to)) {
    ...
    if (catId == 'discount') {
      ...
      final res = calculateDiscount(originalPrice: price, discountPercentage: percent);
      state = state.copyWith(resultValue: 'Final: ${_formatResult(res.finalPrice)} (Saved: ${_formatResult(res.amountSaved)})');
      return;
    }
    ...
    if (catId == 'gst') {
      ...
      final res = calculateGst(amount: amt, gstPercentage: percent, addGst: state.addGst);
      state = state.copyWith(resultValue: 'Total: ${_formatResult(res.totalAmount)} (GST: ${_formatResult(res.gstAmount)})');
      return;
    }
    ...
    if (catId == 'bmi') {
      ...
      final res = calculateBmi(weightKg: w, heightM: h / 100.0); // Assuming height input is in cm
      if (res.bmi > 0) {
        state = state.copyWith(resultValue: 'BMI: ${_formatResult(res.bmi)} - ${res.category}');
      } else {
        state = state.copyWith(resultValue: '');
      }
      return;
    ...
    if (catId == 'numeral') {
      ...
      if (fromBase != null && toBase != null) {
         final res = convertNumeral(value: state.inputValue, fromBase: fromBase, toBase: toBase);
    ...
    if (fromUnit != null && toUnit != null) {
      ...
      if (val != null) {
        final res = convertStandard(value: val, fromUnit: fromUnit, toUnit: toUnit);
        state = state.copyWith(resultValue: _formatResult(res));
      } else {
        state = state.copyWith(resultValue: '');
      }
