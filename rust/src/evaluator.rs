...
use crate::rational::{CalcValue, Rational};
...
pub fn evaluate_expr(expr: &Expr, is_degree: bool, ans_value: f64) -> Result<CalcValue, CalcError> {
    match expr {
        ...
        Expr::Ans => {
            if ans_value.fract() == 0.0 && ans_value >= (i128::MIN as f64) && ans_value <= (i128::MAX as f64) {
                Ok(CalcValue::Rational(Rational::new(ans_value as i128, 1)))
            } else {
                Ok(CalcValue::Float(ans_value))
            }
        }
        Expr::Add(l, r) => Ok(evaluate_expr(l, is_degree, ans_value)?.add(evaluate_expr(r, is_degree, ans_value)?)),
        Expr::Subtract(l, r) => Ok(evaluate_expr(l, is_degree, ans_value)?.sub(evaluate_expr(r, is_degree, ans_value)?)),
        Expr::Multiply(l, r) => Ok(evaluate_expr(l, is_degree, ans_value)?.mul(evaluate_expr(r, is_degree, ans_value)?)),
        Expr::Divide(l, r) => {
            evaluate_expr(l, is_degree, ans_value)?.div(evaluate_expr(r, is_degree, ans_value)?).map_err(|_| CalcError::DivisionByZero)
        }
        Expr::Modulo(l, r) => {
            evaluate_expr(l, is_degree, ans_value)?.modulo(evaluate_expr(r, is_degree, ans_value)?).map_err(|_| CalcError::DivisionByZero)
        }
        Expr::Power(l, r) => {
        ...
        Expr::Asin(e) => {
            ...
            let res = val.asin();
            Ok(CalcValue::Float(if is_degree { res.to_degrees() } else { res }))
        }
        Expr::Acos(e) => {
            let val = evaluate_expr(e, is_degree, ans_value)?.to_float();
            if !(-1.0..=1.0).contains(&val) {
        ...
        Expr::Sinh(e) => Ok(CalcValue::Float(evaluate_expr(e, is_degree, ans_value)?.to_float().sinh())),
        Expr::Cosh(e) => Ok(CalcValue::Float(evaluate_expr(e, is_degree, ans_value)?.to_float().cosh())),
        Expr::Tanh(e) => Ok(CalcValue::Float(evaluate_expr(e, is_degree, ans_value)?.to_float().tanh())),
        Expr::Asinh(e) => Ok(CalcValue::Float(evaluate_expr(e, is_degree, ans_value)?.to_float().asinh())),
        Expr::Acosh(e) => {
            let val = evaluate_expr(e, is_degree, ans_value)?.to_float();
            if val < 1.0 {
